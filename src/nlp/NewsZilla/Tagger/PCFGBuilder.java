...
public class PCFGBuilder {
	...
	//counts the occurrences of a rule, hashed first by left hand side than right hand side
	...
	//Boolean to indicate if a given rule is lexical, hashed the same way as above
	...
	public PCFGBuilder(String filename) {
		...
		//for each rule, we calculate its probability as rule occurrence / lhs occurrence
	...
	int rulecount = 0;
	/**
	 * Counts the occurrences of each rule in a parse tree, and marks them as lexical or not in isLexical
	 * 
	 * 
	 * @param p The ParseTree to count rules for
	 */
	private void countRules(ParseTree p) {
		...
		//This rule is lexical if its children are terminal (handles slightly broader case than required where lexical rule goes to 2  or more words)
	 ...
	 * 
	 * 
	 * @param map the HashMap to operate on
	...
	/**
	 * Increment the value by one in a hash of String to hash of String to double
	 * 
	 * @param map the HashMap to operate on
	 * @param firstKey the key whose map is to be operated on
	 * @param secondKey the key whose value is to be incremented in the sub-HashMap
	 */
	...
	/**
	 * Generates a random sentence based on this PCFG, and cleans up whitespace in the sentence. 
	 * 
	 ...
	 * 
	 * @param lhs the symbol to start from
	...
	private String generateSentence(String lhs) {
		...
		//We select a rule by their probabilities by generating a random float in [0,1] and
		// then iterating over the rules and their probabilities. We choose a rule when
		// the sum of the probabilities of this rule and previous rules is >= to the random float.
		Double probSum = 0.0;
