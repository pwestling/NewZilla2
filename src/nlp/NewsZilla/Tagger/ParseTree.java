 ...
 * structure. A parse tree consists of a label and, if it is not a terminal,
 * children that are also parse trees.
 * 
 * @author Dave Kauchak
 * @version 2/26/2011
 * 
...
public class ParseTree {
	...
	private boolean terminal; // whether or not this is a terminal node in the
								// tree
	private String label; // the label of this node of the tree, e.g. S, NP, VP,
							// etc.

	private ArrayList<ParseTree> children; // the sub parse trees of this node
											// if it's a non-terminal

	/**
	 * Construct a new parse tree from a single line, parenthesized version of
	 * the parse tree.
	 * 
	 * @param treeString
	 *            A single line containing a parenthesized version of a parse
	 *            tree
	 */
	public ParseTree(String treeString) {
		if (treeString.charAt(0) != '(' || treeString.charAt(treeString.length() - 1) != ')') {
		...
		if (treeString.contains("(")) {
				...
				for (int i = 0; i < childrenString.length(); i++) {
					if (childrenString.charAt(i) == '(') {
						...
						if (leftParenCount == 0) {
							...
							// the i+1th character should be a space (or we're
							// off the end)
							start = i + 2;
			...
			// when we get down the the part of speech tag, create the POS node
			// as a non-terminal
			...
			if (m.matches()) {
				...
				children.add(new ParseTree(m.group(2).toLowerCase(), true));
			} else {
	 ...
	 * non-terminal with children yet unattached)
	 * 
	 * @param label
	 *            the constituent label for this node (or the word if it's a
	 *            terminal)
	 * @param terminal
	 *            whether or not this is a terminal
	 */
	 ...
	 * 
	 * @param index
	 *            the index of the child to obtain
	...
	/**
	 * Get the constituent label for this parse tree. If it is a terminal, this
	 * is the word.
	 * 
